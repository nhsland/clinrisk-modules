...
void usage(void) {
	...
	printf(" * XML source: Q54_statins_xml_144_myopathy2_1.xml\n");
	printf(" * STATA dta time stamp: 7 Mar 2010 19:20\n");
	printf(" * C file create date: Fri May 21 09:41:29 BST 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q54_statins_xml_144_myopathy2_1_commandLine age b_corticosteroids b_type2 bmi ethrisk statin_user surv\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_type2 = atoi(argv[3]);
	double bmi = atof(argv[4]);
	int ethrisk = atoi(argv[5]);
	int statin_user = atoi(argv[6]);
	int surv = atoi(argv[7]);

	double score = myopathy2_male(age,b_corticosteroids,b_type2,bmi,ethrisk,statin_user,surv,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
