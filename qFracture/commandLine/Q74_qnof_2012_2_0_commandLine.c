 ...
 * C file create date: Mon  5 Mar 2012 11:14:32 GMT
 */
...
void usage(void) {
	...
	printf(" * XML source: Q74_qnof_2012_2_0.xml\n");
	printf(" * STATA dta time stamp: 1 Mar 2012 11:21\n");
	printf(" * C file create date: Mon  5 Mar 2012 11:14:32 GMT\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q74_qnof_2012_2_0_commandLine age alcohol_cat6 b_antidepressant b_anycancer b_asthmacopd b_corticosteroids b_cvd b_dementia b_endocrine b_epilepsy2 b_falls b_fracture4 b_hrt_oest b_liver b_parkinsons b_ra_sle b_renal b_type1 b_type2 bmi ethrisk smoke_cat surv\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_parkinsons = atoi(argv[15]);
	int b_ra_sle = atoi(argv[16]);
	int b_renal = atoi(argv[17]);
	int b_type1 = atoi(argv[18]);
	int b_type2 = atoi(argv[19]);
	double bmi = atof(argv[20]);
	int ethrisk = atoi(argv[21]);
	int smoke_cat = atoi(argv[22]);
	int surv = atoi(argv[23]);

	double score = nof_female(age,alcohol_cat6,b_antidepressant,b_anycancer,b_asthmacopd,b_corticosteroids,b_cvd,b_dementia,b_endocrine,b_epilepsy2,b_falls,b_fracture4,b_hrt_oest,b_liver,b_parkinsons,b_ra_sle,b_renal,b_type1,b_type2,bmi,ethrisk,smoke_cat,surv,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
