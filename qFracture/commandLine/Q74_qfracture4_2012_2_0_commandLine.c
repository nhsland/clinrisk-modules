 ...
 * C file create date: Mon  5 Mar 2012 11:13:59 GMT
 */
...
void usage(void) {
	...
	printf(" * XML source: Q74_qfracture4_2012_2_0.xml\n");
	printf(" * STATA dta time stamp: 1 Mar 2012 11:21\n");
	printf(" * C file create date: Mon  5 Mar 2012 11:13:59 GMT\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q74_qfracture4_2012_2_0_commandLine age alcohol_cat6 b_antidepressant b_anycancer b_asthmacopd b_corticosteroids b_cvd b_dementia b_endocrine b_epilepsy2 b_falls b_fracture4 b_hrt_oest b_liver b_malabsorption b_parkinsons b_ra_sle b_renal b_type1 b_type2 bmi ethrisk fh_osteoporosis smoke_cat surv\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_corticosteroids = atoi(argv[6]);
	int b_cvd = atoi(argv[7]);
	int b_dementia = atoi(argv[8]);
	int b_endocrine = atoi(argv[9]);
	int b_epilepsy2 = atoi(argv[10]);
	int b_falls = atoi(argv[11]);
	int b_fracture4 = atoi(argv[12]);
	int b_hrt_oest = atoi(argv[13]);
	int b_liver = atoi(argv[14]);
	int b_malabsorption = atoi(argv[15]);
	int b_parkinsons = atoi(argv[16]);
	int b_ra_sle = atoi(argv[17]);
	int b_renal = atoi(argv[18]);
	int b_type1 = atoi(argv[19]);
	int b_type2 = atoi(argv[20]);
	double bmi = atof(argv[21]);
	int ethrisk = atoi(argv[22]);
	int fh_osteoporosis = atoi(argv[23]);
	int smoke_cat = atoi(argv[24]);
	int surv = atoi(argv[25]);

	double score = fracture4_female(age,alcohol_cat6,b_antidepressant,b_anycancer,b_asthmacopd,b_corticosteroids,b_cvd,b_dementia,b_endocrine,b_epilepsy2,b_falls,b_fracture4,b_hrt_oest,b_liver,b_malabsorption,b_parkinsons,b_ra_sle,b_renal,b_type1,b_type2,bmi,ethrisk,fh_osteoporosis,smoke_cat,surv,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
