...
void usage(void) {
	...
	printf(" * XML source: Q74_qfracture4_2012_2_1.xml\n");
	printf(" * STATA dta time stamp: 1 Mar 2012 11:21\n");
	printf(" * C file create date: Mon  5 Mar 2012 11:14:11 GMT\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q74_qfracture4_2012_2_1_commandLine age alcohol_cat6 b_antidepressant b_anycancer b_asthmacopd b_carehome b_corticosteroids b_cvd b_dementia b_epilepsy2 b_falls b_fracture4 b_liver b_malabsorption b_parkinsons b_ra_sle b_renal b_type1 b_type2 bmi ethrisk fh_osteoporosis smoke_cat surv\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_malabsorption = atoi(argv[14]);
	...
	int fh_osteoporosis = atoi(argv[22]);
	int smoke_cat = atoi(argv[23]);
	int surv = atoi(argv[24]);

	double score = fracture4_male(age,alcohol_cat6,b_antidepressant,b_anycancer,b_asthmacopd,b_carehome,b_corticosteroids,b_cvd,b_dementia,b_epilepsy2,b_falls,b_fracture4,b_liver,b_malabsorption,b_parkinsons,b_ra_sle,b_renal,b_type1,b_type2,bmi,ethrisk,fh_osteoporosis,smoke_cat,surv,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
