...
void usage(void) {
	...
	printf(" * XML source: Z3_value_predictor_ratio_1.xml\n");
	printf(" * STATA dta time stamp: 2 Sep 2009 08:55\n");
	printf(" * C file create date: Tue Apr 27 10:48:48 BST 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Z3_value_predictor_ratio_1_commandLine age b_cvd b_treatedhyp b_type2 ethrisk smok\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = ratio_male(age,b_cvd,b_treatedhyp,b_type2,ethrisk,smok,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
