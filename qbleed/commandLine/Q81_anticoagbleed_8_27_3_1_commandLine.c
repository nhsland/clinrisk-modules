 ...
 * C file create date: Tue 29 Jul 2014 14:05:51 BST
 */
...
void usage(void) {
	...
	printf(" * XML source: Q81_anticoagbleed_8_27_3_1.xml\n");
	printf(" * STATA dta time stamp: 25 Jan 2014 20:52\n");
	printf(" * C file create date: Tue 29 Jul 2014 14:05:51 BST\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q81_anticoagbleed_8_27_3_1_commandLine age alcohol_cat6 b_AF b_anticonvulsant b_antidepressant b_antiplat b_liverpancreas b_platlet b_treatedhyp b_varices bmi ethrisk exp_anticoagulant prior_anybleed smoke_cat surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_liverpancreas = atoi(argv[7]);
	int b_platlet = atoi(argv[8]);
	int b_treatedhyp = atoi(argv[9]);
	int b_varices = atoi(argv[10]);
	double bmi = atof(argv[11]);
	int ethrisk = atoi(argv[12]);
	int exp_anticoagulant = atoi(argv[13]);
	int prior_anybleed = atoi(argv[14]);
	int smoke_cat = atoi(argv[15]);
	int surv = atoi(argv[16]);
	double town = atof(argv[17]);

	double score = Q81_anticoagbleed_8_27_3_male(age,alcohol_cat6,b_AF,b_anticonvulsant,b_antidepressant,b_antiplat,b_liverpancreas,b_platlet,b_treatedhyp,b_varices,bmi,ethrisk,exp_anticoagulant,prior_anybleed,smoke_cat,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
