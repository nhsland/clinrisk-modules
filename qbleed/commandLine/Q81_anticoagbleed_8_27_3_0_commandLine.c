...
void usage(void) {
	...
	printf(" * XML source: Q81_anticoagbleed_8_27_3_0.xml\n");
	printf(" * STATA dta time stamp: 25 Jan 2014 20:52\n");
	printf(" * C file create date: Tue 29 Jul 2014 14:05:50 BST\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q81_anticoagbleed_8_27_3_0_commandLine age alcohol_cat6 b_AF b_anticonvulsant b_antidepressant b_antiplat b_liverpancreas b_platlet b_treatedhyp b_varices bmi ethrisk exp_anticoagulant prior_anybleed smoke_cat surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = Q81_anticoagbleed_8_27_3_female(age,alcohol_cat6,b_AF,b_anticonvulsant,b_antidepressant,b_antiplat,b_liverpancreas,b_platlet,b_treatedhyp,b_varices,bmi,ethrisk,exp_anticoagulant,prior_anybleed,smoke_cat,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
