...
void usage(void) {
	...
	printf(" * \n");
	printf(" * This is part of bmiPredictor (http://svn.clinrisk.co.uk/opensource/bmiPredictor).\n");
	printf(" * \n");
	printf(" * bmiPredictor is free software: you can redistribute it and/or modify\n");
	printf(" * it under the terms of the GNU Lesser General Public License as published by\n");
	printf(" * the Free Software Foundation, either version 3 of the License, or\n");
	printf(" * (at your option) any later version.\n");
	printf(" * \n");
	printf(" * bmiPredictor is distributed in the hope that it will be useful,\n");
	printf(" * but WITHOUT ANY WARRANTY; without even the implied warranty of\n");
	printf(" * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the\n");
	printf(" * GNU Lesser General Public License for more details.\n");
	printf(" * \n");
	printf(" * You should have received a copy of the GNU Lesser General Public License\n");
	printf(" * along with bmiPredictor.  If not, see <http://www.gnu.org/licenses/>.\n");
	printf(" * \n");
	...
	printf(" * http://svn.clinrisk.co.uk/opensource/bmiPredictor.\n");
	...
	printf(" * XML source: Z3_value_predictor_bmi_0.xml\n");
	printf(" * STATA dta time stamp: 2 Sep 2009 08:55\n");
	printf(" * C file create date: Tue Apr 27 10:22:31 BST 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Z3_value_predictor_bmi_0_commandLine age b_cvd b_treatedhyp b_type2 ethrisk smok\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = bmi_female(age,b_cvd,b_treatedhyp,b_type2,ethrisk,smok,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
