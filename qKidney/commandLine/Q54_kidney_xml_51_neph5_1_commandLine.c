...
void usage(void) {
	...
	printf(" * XML source: Q54_kidney_xml_51_neph5_1.xml\n");
	printf(" * STATA dta time stamp: 25 Mar 2010 07:28\n");
	printf(" * C file create date: Wed Nov 10 14:00:12 GMT 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q54_kidney_xml_51_neph5_1_commandLine age b_CCF b_cvd b_pvd b_ra b_treatedhyp b_type1 b_type2 bmi ethrisk fh_kidney sbp smoke_cat surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_treatedhyp = atoi(argv[6]);
	int b_type1 = atoi(argv[7]);
	int b_type2 = atoi(argv[8]);
	double bmi = atof(argv[9]);
	int ethrisk = atoi(argv[10]);
	int fh_kidney = atoi(argv[11]);
	double sbp = atof(argv[12]);
	int smoke_cat = atoi(argv[13]);
	int surv = atoi(argv[14]);
	double town = atof(argv[15]);

	double score = neph5_male(age,b_CCF,b_cvd,b_pvd,b_ra,b_treatedhyp,b_type1,b_type2,bmi,ethrisk,fh_kidney,sbp,smoke_cat,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
