...
void usage(void) {
	...
	printf(" * XML source: Q54_kidney_xml_51_neph3_0.xml\n");
	printf(" * STATA dta time stamp: 25 Mar 2010 07:28\n");
	printf(" * C file create date: Wed Nov 10 13:59:22 GMT 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q54_kidney_xml_51_neph3_0_commandLine age b_CCF b_cvd b_nsaid b_pvd b_ra b_renalstones b_sle b_treatedhyp b_type1 b_type2 bmi ethrisk fh_kidney sbp smoke_cat surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_nsaid = atoi(argv[4]);
	int b_pvd = atoi(argv[5]);
	int b_ra = atoi(argv[6]);
	int b_renalstones = atoi(argv[7]);
	int b_sle = atoi(argv[8]);
	int b_treatedhyp = atoi(argv[9]);
	int b_type1 = atoi(argv[10]);
	int b_type2 = atoi(argv[11]);
	double bmi = atof(argv[12]);
	int ethrisk = atoi(argv[13]);
	int fh_kidney = atoi(argv[14]);
	double sbp = atof(argv[15]);
	int smoke_cat = atoi(argv[16]);
	int surv = atoi(argv[17]);
	double town = atof(argv[18]);

	double score = neph3_female(age,b_CCF,b_cvd,b_nsaid,b_pvd,b_ra,b_renalstones,b_sle,b_treatedhyp,b_type1,b_type2,bmi,ethrisk,fh_kidney,sbp,smoke_cat,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
