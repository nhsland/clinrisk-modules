...
void usage(void) {
	...
	printf(" * XML source: Q54_kidney_xml_51_neph3_1.xml\n");
	printf(" * STATA dta time stamp: 25 Mar 2010 07:28\n");
	printf(" * C file create date: Wed Nov 10 13:59:40 GMT 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q54_kidney_xml_51_neph3_1_commandLine age b_CCF b_cvd b_nsaid b_pvd b_ra b_treatedhyp b_type1 b_type2 bmi ethrisk fh_kidney sbp smoke_cat surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	int ethrisk = atoi(argv[11]);
	int fh_kidney = atoi(argv[12]);
	double sbp = atof(argv[13]);
	int smoke_cat = atoi(argv[14]);
	int surv = atoi(argv[15]);
	double town = atof(argv[16]);

	double score = neph3_male(age,b_CCF,b_cvd,b_nsaid,b_pvd,b_ra,b_treatedhyp,b_type1,b_type2,bmi,ethrisk,fh_kidney,sbp,smoke_cat,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
