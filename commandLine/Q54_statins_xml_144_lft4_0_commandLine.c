...
void usage(void) {
	...
	printf(" * XML source: Q54_statins_xml_144_lft4_0.xml\n");
	printf(" * STATA dta time stamp: 7 Mar 2010 19:20\n");
	printf(" * C file create date: Fri May 21 09:41:25 BST 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q54_statins_xml_144_lft4_0_commandLine age b_corticosteroids b_cvd b_ra b_treatedhyp b_type1 b_type2 bmi ethrisk smoke_cat statin_user surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	int statin_user = atoi(argv[11]);
	int surv = atoi(argv[12]);
	double town = atof(argv[13]);

	double score = lft4_female(age,b_corticosteroids,b_cvd,b_ra,b_treatedhyp,b_type1,b_type2,bmi,ethrisk,smoke_cat,statin_user,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
