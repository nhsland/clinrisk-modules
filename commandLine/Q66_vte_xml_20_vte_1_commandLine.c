 ...
 * C file create date: Mon 15 Aug 2011 18:51:10 BST
 */
...
void usage(void) {
	...
	printf(" * XML source: Q66_vte_xml_20_vte_1.xml\n");
	printf(" * STATA dta time stamp: 20 Mar 2011 13:10\n");
	printf(" * C file create date: Mon 15 Aug 2011 18:51:10 BST\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q66_vte_xml_20_vte_1_commandLine age antipsychotics_current b_CCF b_anycancer b_copd b_ibd b_renal b_varicosevein bmi smoke_cat surv z1_admit\n");
}
...
int main (int argc, char *argv[]) {
	...
	int z1_admit = atoi(argv[12]);

	double score = vte_male(age,antipsychotics_current,b_CCF,b_anycancer,b_copd,b_ibd,b_renal,b_varicosevein,bmi,smoke_cat,surv,z1_admit,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
