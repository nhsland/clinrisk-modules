 ...
 * C file create date: Fri 11 Sep 2015 10:36:39 BST
 */
...
void usage(void) {
	...
	printf(" * XML source: Q88_vte_2014_1.xml\n");
	printf(" * STATA dta time stamp: 17 Jun 2014 22:53\n");
	printf(" * C file create date: Fri 11 Sep 2015 10:36:39 BST\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q88_vte_2014_1_commandLine age b_CCF b_admit b_antipsychotic b_anycancer b_copd b_ibd b_renal b_varicosevein bmi smoke_cat surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_copd = atoi(argv[6]);
	int b_ibd = atoi(argv[7]);
	int b_renal = atoi(argv[8]);
	int b_varicosevein = atoi(argv[9]);
	double bmi = atof(argv[10]);
	int smoke_cat = atoi(argv[11]);
	int surv = atoi(argv[12]);
	double town = atof(argv[13]);

	double score = vte_male(age,b_CCF,b_admit,b_antipsychotic,b_anycancer,b_copd,b_ibd,b_renal,b_varicosevein,bmi,smoke_cat,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
